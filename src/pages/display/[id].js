...
const downloadfile = (indice,filename)=>{
  ...
  return (
    ...
    <div>
        <div class=" h-[70px] p-2 flex space-x-64 bg-slate-800">
            <div class="mt-2 flex">
                ...
                <p class="font-Cairo text-white">{file.filename}</p>
            </div>
            <div onClick={()=>{downloadfile(file.id,file.filename)}} class="bg-[#0070f0] cursor-pointer ml-5 w-40 h-11 mt-2 text-center rounded-sm">
                <a  class="text-xl font-Cairo mt-4 font-light text-cyan-50">تحميل</a>
            </div>
        </div>
        <div>

            {data.type == "pdf" ?
                <iframe src="{{asset('storage/'.$dt->path)}}" width="100%" height="500px">  </iframe>
                :
                <Image src={url+"/storage/"+file.path} width="600" height="400" alt="diplay img"/>
                }
        </div>
    </div>
    <div class="w-[700px] mt-3">
        <div>
            <h2 class="font-Cairo font-medium text-xl text-[#55bc74] text-right">:الوصف</h2>
        </div>
        <div>
           {
            data.description ?
                <p class="font-Cairo"> {data.description} </p>
                :
                <p class="font-Cairo font-light text-center"> ... لا يوجد أي وصف</p>
          }
